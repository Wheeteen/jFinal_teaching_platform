...
public class Teacher extends Model<Teacher> {
    ...
    public boolean modifyUserInfo(String id, String file_id, String email, String phone) {
    	...
    	if(file_id != null) {
    		...
    		if(phone != null) {
    			return Db.update("update tea_info set imgUrl=?, email = ?, phone = ? where id=?",fileUrl, email, phone, id) > 0;
    		} else {
    		...
    		if(phone != null) {
    			return Db.update("update tea_info set email = ?, phone = ? where id=?", email, phone, id) > 0;
    		} else {
